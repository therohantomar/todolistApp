...
import { toggleImportant } from "../utils/todoSlice";
...
const TodoCard = (todo) => {
  ...
  return (
    <div className="w-max h-max  p-2 flex flex-col bg-black  text-gray-200 mx-2 border-2 border-gray-500 rounded-md ">
     ...
     <span className="flex gap-2">{todo.isImportant?null:<button className=" rounded-md text-yellow-500 w-max" onClick={()=>dispatch(toggleImportant(todo))}><MdLabelImportant/></button>} 
     <button className=" rounded-md text-yellow-500 w-max" onClick={()=>dispatch(toggleImportant())}><AiOutlineCheckSquare/></button>
     <button className=" rounded-md text-yellow-500 w-max" onClick={()=>dispatch(toggleImportant())}><BiSolidTrashAlt/></button>
     </span>
