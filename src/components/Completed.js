    ...
    if (todos.length === 0)
      return (
        ...
        <div className="absolute top-1/2 left-1/2 right-1/2 bottom-1/2 z-0 -translate-x-1/2 -translate-y-1/2">
          <h1 className="font-bold w-max sm:text-xl md:text-xl lg:text-xl  xl:text-2xl 2xl:text-2xl  text-yellow-500">No Task's are completed</h1>
          <button className="px-2 my-4 sm:text-md md:text-md lg:text-md  xl:text-xl 2xl:text-xl  w-max font-bold bg-yellow-500 rounded-md text-white"><Link to={"/"}>Get Your tasks</Link></button>
        </div>
        </div>
      );
    return (
