...
function Sidebar() {
  ...
  return (
    <aside data-testid="sidebar"  className="  top-10 bottom-0  w-60 bg-gray-900 text-white shadow-xl">
    <button data-testid="createTodo" onClick={()=>{
      ...
      <ul className="my-10">
       <Link to="/"><li   onClick={()=>dispatch(toggleMenu())} className={`font-bold text-white font-nunito  my-2 py-2 ${!params?"bg-black":"bg-gray-500"} text-center hover:bg-black uppercase flex items-center  `}><BsListTask className="mx-4"/>Alltask  <p className="mx-2 text-sm text-red-600 rounded-full">{todos.length===0?null:todos.length}</p></li></Link>
       <Link to="important?v=important"><li onClick={()=>dispatch(toggleMenu())} className={`font-bold font-nunito text-white  my-2 ${params==="important"?"bg-black":"bg-gray-500"}  py-2 text-center hover:bg-black uppercase flex items-center`}><MdLabelImportant className="mx-4"/>Important <p className="mx-2 text-sm text-red-600 rounded-full">{importantTodos.length===0?null:importantTodos.length}</p></li></Link>
       <Link to="completed?v=completed"><li onClick={()=>dispatch(toggleMenu())} className={`font-bold font-nunito text-white  my-2 ${params==="completed"?"bg-black":"bg-gray-500"}  py-2 text-center hover:bg-black uppercase flex items-center`}><BsFillCheckSquareFill className="mx-4"/>Completed<p className="mx-2 text-sm text-red-600 rounded-full">{completedTodos.length===0?null:completedTodos.length}</p></li></Link>
       <Link to="deleted?v=deleted"><li  onClick={()=>dispatch(toggleMenu())} className={`font-bold font-nunito text-white  my-2 ${params==="deleted"?"bg-black":"bg-gray-500"}  py-2 text-center hover:bg-black uppercase flex items-center`}><FaTrash className="mx-4"/>Deleted<p className="mx-2 text-sm text-red-600 rounded-full">{deletedTodos.length===0?null:deletedTodos.length}</p></li></Link>
      </ul>
    </aside>
  )
}
